...
while [ ${round} -le ${repeat} ]; do
    ...
    lein run test --workload non-reentrant-cp-lock --time-limit ${test_duration}

    if [ $? != '0' ]; then
        echo "non-reentrant-cp-lock test failed"
        exit 1
    fi

    echo "running cp-reentrant-lock test"

    lein run test --workload reentrant-cp-lock --time-limit ${test_duration}

    if [ $? != '0' ]; then
        echo "reentrant-cp-lock test failed"
        exit 1
    fi

    echo "running non-reentrant-fenced-lock test"

    lein run test --workload non-reentrant-fenced-lock --time-limit ${test_duration}

    if [ $? != '0' ]; then
        echo "non-reentrant-fenced-lock test failed"
        exit 1
    fi
